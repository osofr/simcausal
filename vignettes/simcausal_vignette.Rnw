...
\item [{\code{add.node} or \code{D + node}}] provide two equivalent ways of
...
the \code{add.node} function or equivalently the \code{'+'} function,
...
this argument is omitted, the \code{add.node} function assigns the temporal
...
<<message=FALSE, cache=cache_opt>>=
  Xdyn <- sim(Ddyn, actions = c("A1_th0", "A1_th1"), n = 100000, rndseed = 123)
  nrow(Xdyn[["A1_th0"]])
  nrow(Xdyn[["A1_th1"]])
  names(Xdyn)
@
...
<<message=FALSE, cache=cache_opt>>=
  lXdyn <- sim(Ddyn, actions = c("A1_th0", "A1_th1"), n = 100000, wide = FALSE, rndseed = 123)
  head(lXdyn[["A1_th0"]], 5)
@
...
<<message=FALSE, cache=cache_opt>>=
  MSMres <- eval.target(Ddyn, n = 100000, rndseed = 123)
  MSMres$coef
@
...
<<message=FALSE>>=
  ...
  Ddyn <- lDAG
@
% SINCE THE ACTIONS HAVE BEEN DEFINED PREVIOUSLY, DON'T NEED TO SHOW THE CODE BELOW:
% Below code defines two dynamic interventions, $d_1$ and $d_2$, as done previously in \ref{actions2}:
...
<<message=FALSE, cache=cache_opt>>=
  ...
  # resNP <- eval.target(Ddyn, n = 1000000, rndseed = 123)$res
  (psi0.NP <- as.vector(resNP["Diff_Y_"%+%t0]))
@
...
<<message=FALSE, eval=TRUE, cache=cache_opt>>=
  MSM_RD_t <- function(resMSM, t) {
    invlogit <- function(x) 1 / (1 + exp(-x))
    Riskth0 <- invlogit(resMSM["(Intercept)"] + resMSM["as.factor(t)"%+%t])
    Riskth1 <- invlogit(resMSM["(Intercept)"] + resMSM["as.factor(t)"%+%t] +
                            resMSM["theta"] + resMSM["theta:as.factor(t)"%+%t])
  ...
  # resMSM <- eval.target(Ddyn, n = 1000000, rndseed = 123)$coef
  psi0.MSM <- MSM_RD_t(resMSM = resMSM, t = t0)
  all.equal(psi0.NP, psi0.MSM)
@
<<message=FALSE, echo=FALSE, eval=FALSE>>=
  save(list = "psi0.MSM", file = "vignette_dat/repstudy1_psi0.MSM.Rdata")
@
...
<<message=FALSE, echo=FALSE>>=
  library(Hmisc)
  load(file = "vignette_dat/repstudy1_psi0.MSM.Rdata")
  ...
  getrestab <- function(iptw_sims, tmle_sims, origres, modelnm) {
    resrow <- function(sims, estname) {
      data.frame(estname, 
                  ...
                  sprintf("%.4f",mean(sims)-psi0.MSM), # round(((mean(sims)-psi0.MSM)/psi0.MSM)*100,3),
...
<<message=FALSE, results='hide', eval=FALSE>>=
  trueMSM.sc1 <- NULL; reptrue <- 50
  for (i in (1:reptrue)) {
    res.sc1.i <- eval.target(Dact.sc1, n = 500000)$coef
    trueMSM.sc1 <- rbind(trueMSM.sc1, res.sc1.i)
  }

  save(list = "trueMSM.sc1", file = "vignette_dat/trueMSM.sc1.Rdata")
  (trueMSM.sc1 <- apply(trueMSM.sc1, 2, mean))
@
<<message=FALSE,echo=FALSE>>=
  load(file = "vignette_dat/trueMSM.sc1.Rdata")
  trueMSM.sc1 <- apply(trueMSM.sc1, 2, mean)
  trueMSM.sc1
@
...
<<message=FALSE, warning=FALSE, results='hide', eval=FALSE>>=
  ...
  trueMSM.sc3 <- NULL; reptrue <- 50
  for (i in (1:reptrue)) {
    res.sc3.i <- eval.target(Dact.sc3, n = 500000)$coef
    trueMSM.sc3 <- rbind(trueMSM.sc3, res.sc3.i)
  }

  save(list = "trueMSM.sc3", file = "vignette_dat/trueMSM.sc3.Rdata")
  (trueMSM.sc3 <- apply(trueMSM.sc3,2,mean))
@
<<message=FALSE, echo=FALSE>>=
  load(file = "vignette_dat/trueMSM.sc3.Rdata")
  trueMSM.sc3 <- apply(trueMSM.sc3, 2, mean)
  trueMSM.sc3
@
