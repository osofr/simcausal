...
confounder (\code{W3}), one binary exposure (\code{Anode}), and one binary outcome (\code{Y}). This example uses pre-defined \proglang{R} functions \code{rcategor},  \code{rbern}, \code{rnorm} and \code{runif} for sampling from the
