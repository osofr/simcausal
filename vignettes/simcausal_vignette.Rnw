...
as ``actions''. These actions may represent exposure to treatment regimens, the occurrence or non-occurrence of right-censoring events, or of clinical monitoring events (e.g., laboratory measurements based on which treatment decisions may
...
from working marginal structural model (MSM) \citep{robins1998marginal,neugebauer2007}. We demonstrate an application of the
...
We note that the \pkg{simcausal} package differs from other R packages 
\citep{R-lavaan,R-lavaan.survey,R-sem,R-semPLS,R-OpenMx} for conducting simulations based on structural equation modeling 
in the following ways. First, \pkg{simcausal} does not restrict the set of distributions
