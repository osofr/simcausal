...
measurement at a specific time point.
% time-varying random variable. 
The package allows for specification and simulation of counterfactual data under various user-specified interventions (e.g., static, dynamic, deterministic, or stochastic). In particular, the interventions may
...
of counterfactual data (referred to as ``\emph{full data}'') under various
...
features of the distribution of the full data that represent common causal
...
static and time-varying intervention, simulate full data, and calculate various causal parameters based on
...
The following example demonstrates how the function \code{DF.to.longDT} can be used to convert simulated (full or
...
evaluate these target parameters using the full data simulated previously in Section~\ref{simfull2.dyn} and we map
