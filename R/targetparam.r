...
subset_dat_long <- function(dt, t_sel) { # subset full data (in long format) by time-points
	if (is.longfmt(dt)) {
		dtDAG <- attr(dt, "DAG")		
...
# set.targetMSM <- function(DAG, outcome, t, formula, family="quasibinomial", hazard=FALSE, ...,  attr=list()) {	
  	# collect all attributes (must be named)
  	attrs <- list(...)
  	attrs <- append(attrs, attr)
  	attnames <- names(attrs)
...
eval.MSM <- function(DAG, df_full, outnodes, outnode_nms, params.MSM, attrs) {
	...
	# m <- glm(as.formula(form), data=df_combine, family=family, na.action=na.exclude)
