...
nodeform_parsers = function(node_form_call, data.env, user.env) {
  ...
  # ************************************************************************************
  # TO DO: 
  # Extend the same functionality from non_TD to TD parents => Want to allow vectors in user.env to be referenced as uservec[t]
  ...
  # node_form_call <- substitute(node_form_call)
...
eval.nodeform.out <- function(expr.idx, self, data.df, cur.node) {
  ...
  # THIS IS NO LONGER NEEDED, SEE parser in Define_sVar class for easier way to do this:
  # ****************************************************************
  df_varnms <- self$df.names(data.df)$ANCHOR_ALLVARNMS_VECTOR_0
  ...
  # NO LONGER NEEDED, this check is now performed in find_FormVars
  # for (Vname in Vnames) {
...
Define_sVar <- R6Class("Define_sVar",
  ...
  public = list(
    ...
    set.new.exprs = function(exprs_list) {
      ...
      dprint("Final summary measure expression(s): "); dprint(self$exprs_list)
      invisible(self)
    },

    # (NOT USED IN THIS PACKAGE....) BORROW THE NAMING AND MAPPING WHEN SAMPLING MULTIVAR RVs
