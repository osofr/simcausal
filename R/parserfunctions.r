		...
		eval_atomic <- function (x, where = parent.frame()) {
			if (is.atomic(x) || is.name(x)) {
				...
				if ((identical(x[[1]], quote(`[`)) && is.name(x[[2]])) | identical(x[[1]], quote(c))) {
			  		...
			  		} else {	# or 2) all args are atomic - then evalute and return result
	...
	modify_call <- function (x, where = parent.frame()) {
		...
		if (is.atomic(x) || is.name(x)) {
			...
			if (identical(x[[1]], quote(`[`)) && is.name(x[[2]])) {	# reached '[' function, don't need to parse any deeper, return this subtree intact
		  	...
		  	} else {	# non-vectorized fucn needs to be wrapped in vecapply, with args combined as cbind(arg1,arg2,...) for more than one arg
...
# 5) evaluates final expression in a special environment where: 
	#a) variables that have been simulated so far in obs.df are accessible
	#b) the subset vector function '[' is replaces with its specialized version, with syntax TDVar[t_range] for subsetting columns of the observed data by time
	#c) vecapply() function that is a wrapper for apply, converts vector to a 1 col matrix
...
eval_nodeform <- function(expr_str, cur.node, env=parent.frame()) {
	...
	f_tovect <- function(X) {	# turn one column matrix to vector
		...
		# stop("fatal error: expression evaluated to a matrix with more than one column", call.=FALSE)
		X
	}
