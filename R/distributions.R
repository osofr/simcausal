...
#'# Three equivalent ways of creating a multivariate node (just repeating W1 and W2):
#'D <- D + node(c("W1.copy1", "W2.copy1"), distr = "rconst", const = c(W1, W2))
#'D <- D + node(c("W1.copy2", "W2.copy2"), distr = "rconst", const = .(cbind(W1, W2)))
#'D <- D + node(c("W1.copy3", "W2.copy3"), distr = "rconst", const = .(create_mat(W1, W2)))
#'Dset <- set.DAG(D)
